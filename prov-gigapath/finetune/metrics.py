...
class MakeMetrics:
    ...
    def get_metric(self, labels: np.array, probs: np.array):
        ...
        # print("get_metric - labels: ", labels)
        # print("get_metric - probs: ", probs)

        # Check if both classes are present in the labels for roc_auc_score and average_precision_score
        if self.metric in ['auroc', 'auprc']:
            unique_labels = np.unique(labels)
            if len(unique_labels) == 1:
                print(f"Warning: Only one class present in y_true. {self.metric.upper()} score is not defined in that case.")
                return None  # Return None or a default value if both classes are not present
    ...
    def process_preds(self, labels: np.array, probs: np.array):
        ...
        if self.metric in ['bacc', 'acc', 'qwk']:
            ...
            processed_probs = np.argmax(probs, axis=1)
            # print("process_preds - processed_labels: ", processed_labels)
            # print("process_preds - processed_probs: ", processed_probs)
            return processed_labels, processed_probs
        else:
            # print("process_preds - labels: ", labels)
            # print("process_preds - probs: ", probs)
            return labels, probs
